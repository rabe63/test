...
const titleMain = 'Proben Nadeln/Blätter – Massen (nach Jahren)'
...
const GROUP_SPAN = 0.82
// Boxplots erst ab n>=3 zeichnen, kleinere Serien als Einzelwerte-Punkte darstellen
...
function buildYearSplitData() {
  ...
  const filtered = rawData.value.filter(r => {
    ...
    return r.mass != null && !isNaN(r.mass)
  })
  ...
  for (const p of plotsSummary) {
    ...
    for (const y of years) {
      ...
      if (bucket.values.length >= DRAW_MIN_BOXES) {
        ...
        // n<3: explizit keine Box, keine Ausreißer – Darstellung als Einzelwerte
        bucket.box = null
        bucket.outliers = []
        bucket.fences = null
      }
...
// Einzelwerte-Dataset (n<3) – Punkte statt Boxplot
function buildSmallNDataset(visiblePlots) {
  const data = []
  const plotCount = visiblePlots.length || 1
  const step = GROUP_SPAN / plotCount
  const start = -GROUP_SPAN / 2 + step / 2
  ...
  visiblePlots.forEach((p, pi) => {
    ...
    years.forEach((y, yi) => {
      const bucket = ym.get(y)
      if (!bucket || !bucket.values?.length || bucket.n >= DRAW_MIN_BOXES) return
      const relOffset = start + pi * step
      bucket.values.forEach(v => {
        data.push([yi, v, relOffset, p.code, y]) // gleiche Struktur wie Ausreißer
      })
    })
  })
  return data
...
// Custom-Renderer für Punkte (Ausreißer + Einzelwerte n<3)
...
function makeOption() {
  ...
  const boxplotSeries = plotsSummary.map(p => {
    ...
    const data = years.map(y => {
      const b = ym?.get(y)
      if (b?.box && b.n >= DRAW_MIN_BOXES) {
        return { value: b.box, year: y, plotCode: p.code }
      }
      // Kein Boxplot für n<3 -> NaN damit keine Box gezeichnet wird
  ...
  const smallNData = buildSmallNDataset(visibleNow)
  ...
  const outlierSeries = {
    ...
    tooltip: {
      ...
      formatter: p => {
        ...
        if (!plot) return ''
        return `<strong>${plot.code} (${plot.speciesName}) – ${year}</strong><br/>Ausreißer: ${fmt(val)} g`
      }
    },
    z: 10
  }
  ...
  const smallNSeries = {
    ...
    tooltip: {
      ...
      formatter: p => {
        ...
        const plot = plotsSummary.find(pl => pl.code === code)
        if (!plot) return ''
        return `<strong>${plot.code} (${plot.speciesName}) – ${year}</strong><br/>Einzelwert (n&lt;3): ${fmt(val)} g`
      }
    },
    z: 10
  }
  ...
  return {
    ...
    yAxis: { type: 'value', name: 'Masse (g)', scale: true },
    tooltip: {
      trigger: 'item',
      confine: true,
      formatter: params => {
        if (params.seriesType === 'boxplot') {
          ...
          if (!bucket) return `<strong>${plot.code} (${plot.speciesName}) – ${year}</strong><br/>Keine Daten`
          if (!bucket.box || bucket.n < DRAW_MIN_BOXES) {
            // Falls doch ein Tooltip für die "leere" Box getriggert würde, spreche klar von Einzelwerten
            const vals = (bucket.values || []).map(v => fmt(v)).join('<br/>')
            const valsBlock = vals ? `<br/>Einzelwerte (n&lt;3):<br/>${vals} g` : '<br/>Keine Daten'
            return `<div style="min-width:240px">
              <strong>${plot.code} (${plot.speciesName}) – ${year}</strong><br/>
              n: ${bucket.n}${valsBlock}
            </div>`
          }
          const [min, q1, med, q3, max] = bucket.box
          const outs = bucket.outliers || []
          const outsBlock = outs.length ? `<br/>Ausreißer:<br/>${outs.map(v => fmt(v)).join('<br/>')} g` : ''
          return `<div style="min-width:240px">
            <strong>${plot.code} (${plot.speciesName}) – ${year}</strong><br/>
            n: ${bucket.n}<br/>
            min: ${fmt(min)} g<br/>Q1: ${fmt(q1)} g<br/>Median: ${fmt(med)} g<br/>Q3: ${fmt(q3)} g<br/>max: ${fmt(max)} g
            ${outsBlock}
          </div>`
        }
        return ''
  ...
  schedule(() => {
    try {
      ...
      myChart.on('legendselectchanged', evt => {
        ...
        // Nur die Punkt-Daten müssen wir neu berechnen; Boxplot-Sichtbarkeit regelt die Legende selbst.
        const visibleNow = getVisiblePlots()
        const outlierData = buildOutlierDataset(visibleNow)
        const smallNData = buildSmallNDataset(visibleNow)
        ...
        myChart.setOption({
          series: [
            { id: '_outliers_', data: outlierData },
            { id: '_smallN_', data: smallNData }
          ]
        }, false)
  ...
  for (const p of visible) {
    ...
    for (const y of years) {
      ...
      if (hasBox) {
        ...
        // n<3: Stats leer lassen, Werte wie Ausreißer aufführen
        csv.push([
          y,
          `"${p.code}"`,
          `"${plotsData[p.code]?.name || ''}"`,

...
function computeOutlierOffsetUpdates() {
  ...
  // Bandbreite schätzen (ohne convertToPixel)
  const plotWidth = Math.max(8, (chartEl.value?.clientWidth || 800) - GRID_LEFT - GRID_RIGHT)
  const bandPx = Math.max(8, plotWidth / Math.max(1, cats.length))
...
// 3) setOption strikt außerhalb des Renderzyklus anwenden
let offsetTimer = 0
...
function buildChartSeries() {
  ...
  visible.forEach((p) => {
    ...
    // Ausreißer als Scatter (ohne Offsets → zentral über der Kategorie)
    const outData = []
